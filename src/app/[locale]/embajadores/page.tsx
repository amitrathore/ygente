...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'embajadoresPage' });
...
function EmbajadoresPageContent() {
  const t = useTranslations('embajadoresPage');
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {steps.map((step, index) => (
              <div key={index} className="text-center">
                <div className="w-16 h-16 rounded-full bg-brand-teal text-white text-2xl font-bold flex items-center justify-center mx-auto mb-4">
                  {step.icon}
                </div>
